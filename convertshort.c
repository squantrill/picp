...
typedef struct
{
	...
	unsigned char			*defx;		// extended definition (initialization) (always 16 bytes long)
...
void readPIC(void)
{
	...
	fprintf(fpout, "\t%x", picdev.cpbits);
	fprintf(fpout, " %x", picdev.wdbit);
	fprintf(fpout, " %x", picdev.wordalign);
	fprintf(fpout, " %x", picdev.cfgmem);
	fprintf(fpout, " %x %x", picdev.idaddr, picdev.idsize);
	fprintf(fpout, " %x", picdev.eeaddr);
